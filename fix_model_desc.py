...
file_path = r'src/utils/modelDescriptions.ts'

new_wood_content = """  'wood': {
    'TSY-100': {
      tr: {
...
# Regex
# Matches 'wood': { ... until ... 'glass': {
# We capture the 'wood': { ... } block.
# Assuming 'glass': { follows.
pattern = r"(\s*'wood':\s*\{.*?)(\s*'glass':\s*\{)"
...
if match:
    print("Found 'wood' to 'glass' block.")
    old_block = match.group(1)
    
    # We replace the old block with new_wood_content
    # And KEEP the 'glass': { part (group 2)
    # The new_wood_content should end with },\n
    
    # Check if new_wood_content ends with newline
    if not new_wood_content.endswith('\n'):
        new_wood_content += '\n'
        
    replacement = new_wood_content + match.group(2)
    
    # We replace the Full Match (wood...glass) with (new_wood...glass)
    # This effectively replaces wood... with new_wood...
    
    new_file_content = content.replace(match.group(0), replacement)
    ...
    # Debug: print snippet
    start = content.find("'wood'")
    if start != -1:
        print(f"File content around 'wood':\n{content[start:start+200]}...")
    else:
        print("'wood' not found in file.")
