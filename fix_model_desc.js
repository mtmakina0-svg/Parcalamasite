...
const fs = require('fs');
const path = 'src/utils/modelDescriptions.ts';

const newWoodContent = `  'wood': {
    'TSY-100': {
      tr: {
...
try {
    ...
    // Regex: Match 'wood': { ... up to but NOT including 'glass': {
    // Use [\s\S] for dotall.
    const regex = /('wood':\s*\{[\s\S]*?)(?=\s*'glass':\s*\{)/;

    if (regex.test(content)) {
        console.log("Found match.");
        // Replace with new_woodContent + newline
        const newFileContent = content.replace(regex, newWoodContent + '\n');
        fs.writeFileSync(path, newFileContent, 'utf8');
        console.log("Replaced successfully.");
    } else {
        console.log("No match found.");
        console.log("Snippet near 'wood':", content.substring(content.indexOf("'wood'"), content.indexOf("'wood'") + 100));
    }
} catch (err) {
